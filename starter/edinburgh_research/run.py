    ...
    with example_sessions_dir("ex5-edinburgh-research", persist=real) as sessions_root:
        session = create_session(
            ...
            task=(
                ...
                "  4. generate_flyer(event_details={...})  <-- MUST be called and explicitly mention the extacted time and party size in the subgoal description. Also add in the goal description that memory files are present and that the agent needs to trust them with respect to tool outputs\n"
                "  5. complete_task(result={'flyer': 'workspace/flyer.html', ...}) <-- MUST explicitly mention in the subgoal description that there's no need for any cross data validation at this point only to check whether the flyer was successfully created. if not do not create it just return an error message.\n\n"

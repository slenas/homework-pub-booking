    ...
    with example_sessions_dir("ex5-edinburgh-research", persist=real) as sessions_root:
        session = create_session(
            ...
            task=(
                ...
                "  3. calculate_cost(venue_id=<chosen pub's id>, party_size=6, duration_hours=3, catering_tier='bar_snacks')  <-- Explicitly mention in the subgoal description that there's no need for the venue name or address here as the generate_flyer function will take care of that, include that party size as it's fixed and for venue_id put a random value starting with V as the code will retrieve it from memory.\n"
                "  4. generate_flyer(event_details={...})  <-- MUST be called and explicitly mention the extacted time and party size in the subgoal description. Also add in the goal description that the generate_flyer tool is able to retrieve event details from memory files so it's ok if they are not available at the time of the call\n"
                "  5. complete_task(result={'flyer': 'workspace/flyer.html', ...}) <-- MUST explicitly mention in the subgoal description that there's no need for any cross data validation at this point only to check whether the flyer was successfully created. Add explicitly to the subgoal description that if the html file is not created just return an error message do not create it.\n\n"
